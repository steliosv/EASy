...
/**
 * @file //<>//
 * @brief E.A.SY. Application. This contains the canvas class of the Voltometer
 * reading plots of EASY app
 * @author Stelios Voutsinas (stevo)
 * @bug No known bugs.
 */
/**
 * @brief This contains the canvas class of the voltometer plots of EASY app
 */
@SuppressWarnings("ClassWithoutLogger")
class VoltCanvas extends Canvas implements SensorEventListener {

    private static final Logger LOG = Logger.getLogger(VoltCanvas.class.getName());

    private final RollingLine2DTrace trace;
    private Graph2D graph;
    private final SampleConverter converter = new VoltageConverter();
    //lock
    ...
    public void sensorEvent(SensorEvent e) {
        ...
        double value = converter.convert((int) e.getVoltage());
        synchronized (lock) {
            sampleCount++;
            sampleValue += value;
    ...
    public void setup(PApplet p) {
        ...
        graph.setYAxisMax(6.0f);
        graph.setYAxisMin(4.0f);
        graph.setYAxisLabel("Volts (V)");
     ...
     */
    private class VoltEq implements ILine2DEquation {
