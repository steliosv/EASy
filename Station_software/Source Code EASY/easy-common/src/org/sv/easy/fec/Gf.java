...
/**
 * @brief This contains the methods for the operations in the Galois fields
 */
public class Gf {
    ...
    /**
     * @brief Substraction in Galois Fields
     * @param x - left operand
     * @param y - right operand
     * @return x - y
     */
     ...
     * @param x - scalar
     */
    public static void poly_scale(Poly p, Poly newp, int x) {
     ...
     * @param newp - destination polynomial
     */
    public static void poly_add(Poly p, Poly q, Poly newp) {
        newp.length = Math.max(p.length, q.length);

        Arrays.fill(newp.ptr(), 0, newp.length, 0);

        for (int i = 0; i < p.length; i++) {
            newp.ptr()[i + newp.length - p.length] = p.at(i);
        }

        for (int i = 0; i < q.length; i++) {
            newp.ptr()[i + newp.length - q.length] ^= q.at(i);
        }
    }
     ...
     * @param newp - destination polynomial
     */
    public static void poly_mul(Poly p, Poly q, Poly newp) {
        ...
        for (int j = 0; j < q.length; j++) {
            for (int i = 0; i < p.length; i++) {
                newp.ptr()[i + j] ^= mul(p.at(i), q.at(j));
     ...
     * @param newp - destination polynomial
     */
    public static void poly_div(Poly p, Poly q, Poly newp) {
        if (p.ptr() != newp.ptr()) {
            System.arraycopy(p.ptr(), 0, newp.ptr(), 0, p.length);
        }
        ...
        for (int i = 0; i < (p.length - (q.length - 1)); i++) {
            ...
            if (coef != 0) {
                for (int j = 1; j < q.length; j++) {
                    if (q.at(j) != 0) {
                        newp.ptr()[i + j] ^= mul(q.at(j), coef);
                    }
                }
            }
        }

        int sep = (p.length - (q.length - 1));
        System.arraycopy(newp.ptr(), sep, newp.ptr(), 0, (newp.length - sep));
     ...
     */
    public static int poly_eval(Poly p, int x) {
        int y = p.at(0);
